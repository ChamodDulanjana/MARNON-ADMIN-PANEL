...
interface PaginatedCategoryResponseDTO {
    categoryList: CategoryGetDTO[],
    meta: metaProps,
}
...
const ViewCategory = () => {
    ...
    const {
        isLoading,
        isError,
        data = {categoryList: [], meta: {total: 0,  page, limit, totalPages: 0}}
    } = useQuery<PaginatedCategoryResponseDTO>({
        queryKey: ['category-by-id', page, limit, Array.from(selectedStatusKeys)],
        queryFn: () =>
        {
            if (isActiveSelected && !isInactiveSelected) {
                return searchCategoryByPagination(searchText, 'active', { page, limit });
            } else if (isInactiveSelected && !isActiveSelected) {
                return searchCategoryByPagination(searchText, 'inactive', { page, limit });
            } else {
                return searchCategoryByPagination(searchText, 'all', { page, limit });
            }
        },
    });
    ...
    const searchTextHandler = async () => {
        if (isActiveSelected && !isInactiveSelected) {
            ...
            await queryClient.fetchQuery({
                queryKey: ['category-by-id', page, limit, Array.from(selectedStatusKeys)],
                queryFn: () => searchCategoryByPagination(searchText, 'active', {page, limit}),
            });
        } else if (isInactiveSelected && !isActiveSelected) {
            // Search only in inactive categories
            await queryClient.fetchQuery({
                queryKey: ['category-by-id', page, limit, Array.from(selectedStatusKeys)],
                queryFn: () => searchCategoryByPagination(searchText, 'inactive', {page, limit}),
            });
        } else {
            // Search in all categories
            await queryClient.fetchQuery({
                queryKey: ['category-by-id', page, limit, Array.from(selectedStatusKeys)],
                queryFn: () => searchCategoryByPagination(searchText, 'all', {page, limit}),
            });
        }
    }
    ...
    const list = useAsyncList({
        async load() {return {items: data.categoryList};},
        async sort({sortDescriptor}) {
            return {
                items: data.categoryList.sort((a, b) => {
    ...
    return (
        <div className="w-full">
            ...
            >
                ...
                >
                    {data.categoryList.map((row: CategoryGetDTO) =>
                        <TableRow key={row.id}>
