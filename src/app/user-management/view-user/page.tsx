...
import {changeUserStatus, searchUserByPagination} from "@/services/userService.ts";
import { CiSearch } from "react-icons/ci";
import {useEffect, useState} from "react";
import {useQuery, useQueryClient} from "@tanstack/react-query";
import type {UserGetDTO} from "@/models/UserGetDTO.ts";
...
const ViewUser = () => {
    ...
    const {
        isLoading,
        isError,
        data = {userList: [], meta: {total: 0,  page, limit, totalPages: 0}}
    } = useQuery<PaginatedUserResponseDTO>({
        queryKey: ['user-by-id', page, limit, Array.from(selectedStatusKeys)],
        queryFn: () =>
        {
            if (isActiveSelected && !isInactiveSelected) {
                return searchUserByPagination(searchText, 'active', { page, limit });
            } else if (isInactiveSelected && !isActiveSelected) {
                return searchUserByPagination(searchText, 'inactive', { page, limit });
            } else {
                return searchUserByPagination(searchText, 'all', { page, limit });
            }
        },
    });
    ...
    const searchTextHandler = async () => {
        if (isActiveSelected && !isInactiveSelected) {
            // Search only in active categories
            await queryClient.fetchQuery({
                queryKey: ['user-by-id', page, limit, Array.from(selectedStatusKeys)],
                queryFn: () => searchUserByPagination(searchText, 'active', {page, limit}),
            });
        } else if (isInactiveSelected && !isActiveSelected) {
            // Search only in inactive categories
            await queryClient.fetchQuery({
                queryKey: ['user-by-id', page, limit, Array.from(selectedStatusKeys)],
                queryFn: () => searchUserByPagination(searchText, 'inactive', {page, limit}),
            });
        } else {
            // Search in all categories
            await queryClient.fetchQuery({
                queryKey: ['user-by-id', page, limit, Array.from(selectedStatusKeys)],
                queryFn: () => searchUserByPagination(searchText, 'all', {page, limit}),
            });
        }
    }
    ...
    const changeStatusHandler = async () => {
        await changeUserStatus(status.sid, status.active ? 0 : 1).then(async (res) => {
            if (res.statusCode === 200) {
                addToast({
                    title: "Success!",
                    description: `User has been ${status.active ? 'deactivated' : 'activated'} successfully.`,
                    color: "success",
                });
    ...
    const list = useAsyncList({
        async load() {return {items: data.userList};},
        async sort({sortDescriptor}) {
            return {
                items: data.userList.sort((a, b) => {
            ...
            >
                <TableHeader>
                    {columns.map((column) =>
                        <TableColumn
                            key={column.key}
                            allowsSorting={['id', 'name', 'createDate', 'createBy', 'modifyDate', 'modifyBy'].includes(column.key)}
                        >
                            {column.label}
                        </TableColumn>
                    )}
                ...
                >
                    {data.userList.map((row: UserGetDTO) =>
                        <TableRow key={row.id}>
